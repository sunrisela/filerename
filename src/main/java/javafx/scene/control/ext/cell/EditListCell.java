...
import javafx.scene.control.ext.skin.ListCellSkin;
 ...
 * @since JavaFX 2.2
 */
public class EditListCell<T> extends ListCell<T> {
    ...
    public static <T> Callback<ListView<T>, ListCell<T>> forListView(final StringConverter<T> converter) {
        return list -> new EditListCell<T>(converter);
    }
     ...
     *      type T.
     */
    public EditListCell(StringConverter<T> converter) {
